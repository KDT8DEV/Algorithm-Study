...
function solution(players, m, k) {
  ...
  for (let hour = 0; hour < 24; hour++) {
    // serverEndTimes에 저장된 값이 현재 hour보다 작거나 같으면 반납된 것. 그렇게 때문엥 필터링 해주면 돼.
    serverEndTimes = serverEndTimes.filter((endTime) => endTime > hour);

    const currentServers = serverEndTimes.length;

    const needServers = Math.floor(players[hour] / m);

    const addServers = Math.max(0, needServers - currentServers);
    if (addServers > 0) {
      totalServersAdded += addServers;

      for (let t = 0; t < addServers; t++) {
