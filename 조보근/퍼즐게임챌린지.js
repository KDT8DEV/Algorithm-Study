...
function solution(diffs, times, limit) {
  function getTotalTime(skillLevel) {
    let total = 0;
    ...
    for (let i = 0; i < diffs.length; i++) {
      ...
      if (difficulty <= skillLevel) {
        ...
        const wrongCount = difficulty - skillLevel;
        total += (current + prevTime) * wrongCount + currentTime;
      }
    }
    return total;
  }
  ...
  let minSkill = 1;
  let maxSkill = Math.max(...diffs);
  let result = maxSkill;

  while (minSkill <= maxSkill) {
    const midSkill = Math.floor((minSkill + maxSkill) / 2);
    const requiredTime = getTotalTime(midSkill);
