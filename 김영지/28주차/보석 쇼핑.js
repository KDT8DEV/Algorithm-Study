...
//=========================================//
let gems = ["AA", "AB", "AC", "AA", "AC"]; // [1, 3]

function solution(gems) {
  let totalCnt = new Set(gems).size; // 전체 보석 종류 수
  let map = new Map(); // 현재 구간 내 보석 개수
  let left = 0;
  let answer = [0, gems.length - 1]; // 초기값

  for (let right = 0; right < gems.length; right++) {
    // right 포인터가 가리키는 보석을 구간에 포함
    map.set(gems[right], (map.get(gems[right]) || 0) + 1);

    // 현재 구간이 모든 종류의 보석을 포함하면
    while (map.size === totalCnt) {
      // 기존 정답보다 더 짧은 구간이면 갱신
      if (right - left < answer[1] - answer[0]) {
        answer = [left, right];
      }

      // left 포인터를 오른쪽으로 옮기기 전에 현재 left 보석 개수 감소
