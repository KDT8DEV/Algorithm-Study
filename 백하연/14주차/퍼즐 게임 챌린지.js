 ...
 * 퍼즐 게임 챌린지
 * 이진 탐색(Binary Search)
 * @param {number[]} diffs 퍼즐의 난이도
 * @param {number[]} times 퍼즐의 소요 시간
 * @param {number} limit 전체 제한 시간
 * @returns 제한 시간 내에 퍼즐을 모두 해결하기 위한 숙련도의 최솟값
 */
/* 풀이 1 : 런타임 에러 */
/*
function solution(diffs, times, limit) {
  // 숙련도(level)이 높을수록 퍼즐을 푸는 시간이 짧다!
  // 30초 안에 모든 퍼즐을 풀 수 있는 가장 낮은 숙련도(level) 구하기
  ...
  function solve(level) {
    ...
    diffs.forEach((diff, index) => {
      ...
      // level >= diff : 퍼즐을 틀리지 않고 time_cur만큼의 시간 소요
      if (level >= diff) {
        totalTime += time_cur;
      } else {
        const wrongCount = diff - level;
        totalTime += (time_cur + time_prev) * wrongCount + time_cur;
      }
    });
  ...
  let low = 1; // 최소 숙련도
  let high = Math.max(...diffs); // 최대 숙련도
  let answer = high;
  while (low <= high) {
...
function solution(diffs, times, limit) {
  function solve(level) {
    let totalTime = 0;
    // forEach는 성능상 for문보다 느릴 수 있고, 중간 멈춤이 어려움
  ...
  let low = 1;
  // 런타임 에러 원인: 배열이 너무 클 때 ...spread 연산자는 스택 오버플로우를 발생시킴.
  // 따라서 reduce를 사용하거나 단순 반복문으로 최댓값을 구해야 합니다.
  let high = diffs.reduce((max, cur) => (cur > max ? cur : max), 0);
  let answer = high;
